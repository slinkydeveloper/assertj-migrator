...
public class Predicates {
  ...
  private static final ResolvedType THROWABLE = new ReferenceTypeImpl(TYPE_SOLVER.solveType(Throwable.class.getName()),
                                                                      TYPE_SOLVER);
  private static final ResolvedType COLLECTION = new ReferenceTypeImpl(TYPE_SOLVER.solveType(Collection.class.getName()),
                                                                       TYPE_SOLVER);
  ...
  private static final ResolvedType OPTIONAL = new ReferenceTypeImpl(TYPE_SOLVER.solveType(Optional.class.getName()),
                                                                     TYPE_SOLVER);
  ...
  public static Predicate<Expression> methodScopeMatches(Predicate<Expression> predicate) {
    return expression -> expression.isMethodCallExpr()
                         && expression.asMethodCallExpr().getScope().map(predicate::test).orElse(false);
  }
  ...
  public static Predicate<Expression> methodDeclaredIn(Class<?> clazz) {
    return expr -> expr.isMethodCallExpr()
                   && clazz.getName().equals(expr.asMethodCallExpr().resolve().declaringType().getQualifiedName());
  }
  ...
  public static Predicate<Expression> binaryOperatorMatches(BinaryExpr.Operator operator, Predicate<Expression> left,
